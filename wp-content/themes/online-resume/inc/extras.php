 ...
 * @return array
 */
function onlineresume_body_classes( $classes ) {
...
add_filter( 'body_class', 'onlineresume_body_classes' );
...
if (!function_exists('onlineresume_str_replace_assoc')) {

    /**
     * onlineresume_str_replace_assoc
     * @param  array $replace
     * @param  array $subject
     * @return array
     */
    function onlineresume_str_replace_assoc(array $replace, $subject) {
 ...
 */
function onlineresume_add_to_cart_fragment( $fragments ) {
	...
	<a class="nav-link cart-icon-wrap header-cart" href="javascript:void(0);" role="button" data-bs-toggle="collapse" data-bs-target="#Cart">
		...
		?>	
			<span><?php echo esc_html__('0','online-resume'); ?></span>
		<?php
		}
		?>
	</a>
...
add_filter( 'woocommerce_add_to_cart_fragments', 'onlineresume_add_to_cart_fragment' );
...
if ( ! function_exists( 'onlineresume_breadcrumb_title' ) ) {
	function onlineresume_breadcrumb_title() {
		...
		elseif ( is_day() ) : 
			...
			printf( __( 'Daily Archives: %s', 'online-resume' ), get_the_date() );
		
		elseif ( is_month() ) :
		
			printf( __( 'Monthly Archives: %s', 'online-resume' ), (get_the_date( 'F Y' ) ));
			
		elseif ( is_year() ) :
		
			printf( __( 'Yearly Archives: %s', 'online-resume' ), (get_the_date( 'Y' ) ) );
			
		elseif ( is_category() ) :
		
			printf( __( 'Category Archives: %s', 'online-resume' ), (single_cat_title( '', false ) ));

		elseif ( is_tag() ) :
		
			printf( __( 'Tag Archives: %s', 'online-resume' ), (single_tag_title( '', false ) ));
			
		elseif ( is_404() ) :

			printf( __( 'Error 404', 'online-resume' ));
			
		elseif ( is_author() ) :
		
			printf( __( 'Author: %s', 'online-resume' ), (get_the_author( '', false ) ));		
...
function onlineresume_breadcrumbs() {
	
	$showOnHome	= esc_html__('1','online-resume'); 	// 1 - Show breadcrumbs on the homepage, 0 - don't show
	$delimiter 	= '';   // Delimiter between breadcrumb
	$home 		= esc_html__('Home','online-resume'); 	// Text for the 'Home' link
	$showCurrent= esc_html__('1','online-resume'); // Current post/page title in breadcrumb in use 1, Use 0 for don't show
    ...
    if ( is_category() ) 
	{
		...
		echo $before . esc_html__('Archive by category','online-resume').' "' . esc_html(single_cat_title('', false)) . '"' .$after;
		
	} 
	
	elseif ( is_search() ) 
	{
		echo $before . esc_html__('Search results for','online-resume').' "' . esc_html(get_search_query()) . '"' . $after;
	} 
	...
	elseif ( is_tag() ) 
	{
		echo $before . esc_html__('Posts tagged ','online-resume').' "' . esc_html(single_tag_title('', false)) . '"' . $after;
	} 
	
	elseif ( is_author() ) {
		global $author;
		$userdata = get_userdata($author);
		echo $before . esc_html__('Articles posted by ','online-resume').'' . $userdata->display_name . $after;
	} 
	
	elseif ( is_404() ) {
		echo $before . esc_html__('Error 404 ','online-resume'). $after;
    }
	
    if ( get_query_var('paged') ) {
		if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo '';
		echo ' ( ' . esc_html__('Page','online-resume') . '' . esc_html(get_query_var('paged')). ' )';
		if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo '';
    }
...
add_action( 'wp_ajax_onlineresume_dismissed_notice_handler', 'onlineresume_ajax_notice_handler' );

/**
 * AJAX handler to store the state of dismissible notices.
 */
function onlineresume_ajax_notice_handler() {
    if ( isset( $_POST['type'] ) ) {
...
add_action( 'wp_ajax_install_act_plugin', 'onlineresume_admin_install_plugin' );

function onlineresume_admin_install_plugin() {
    /**
     * Install Plugin.
     */
